<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.master" Inherits="System.Web.Mvc.ViewPage<ProyectoFinalDisenio.Models.Rol>" %>

<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
    Create
</asp:Content>

<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">

<h2>Create</h2>

<script src="<%: Url.Content("~/Scripts/jquery.validate.min.js") %>" type="text/javascript"></script>
<script src="<%: Url.Content("~/Scripts/jquery.validate.unobtrusive.min.js") %>" type="text/javascript"></script>

<% using (Html.BeginForm()) { %>
    <%: Html.ValidationSummary(true) %>
    <fieldset>
        <legend>Rol</legend>

        <div class="editor-label">
            <%: Html.LabelFor(model => model.idRol) %>
        </div>
        <div class="editor-field">
            <%: Html.EditorFor(model => model.idRol) %>
            <%: Html.ValidationMessageFor(model => model.idRol) %>
        </div>

        <div class="editor-label">
            <%: Html.LabelFor(model => model.nbrRol) %>
        </div>
        <div class="editor-field">
            <%: Html.EditorFor(model => model.nbrRol) %>
            <%: Html.ValidationMessageFor(model => model.nbrRol) %>
        </div>

        <div class="editor-label">
            <%: Html.LabelFor(model => model.descripcionRol) %>
        </div>
        <div class="editor-field">
            <%: Html.EditorFor(model => model.descripcionRol) %>
            <%: Html.ValidationMessageFor(model => model.descripcionRol) %>
        </div>

        <p>
            <input type="submit" value="Create" />
        </p>
    </fieldset>
<% } %>

<div>
    <%: Html.ActionLink("Back to List", "Index") %>
</div>

</asp:Content>
